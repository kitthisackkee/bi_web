...
@section('content')
    <section class="content-header">
        <div class="container-fluid">
          <div class="row mb-2">
            <div class="col-sm-6">
              <h1>{{__('lang.class_room')}}</h1>
            </div>
            <div class="col-sm-6">
              <ol class="breadcrumb float-sm-right">
                <li class="breadcrumb-item"><a href="{{route('dashboard.index')}}">{{__('lang.home')}}</a></li>
                <li class="breadcrumb-item active">{{__('lang.class_room')}}</li>
              </ol>
            </div>
          </div>
        </div>
            ...
            <div class="col-12">
              <div class="card">
                <div class="card-header">
                  <button class="btn btn-primary btn-sm" data-toggle="modal" data-target="#addClassRoomModal">{{__('lang.add')}}</button>
                </div>
                <div class="card-body">
                    ...
                    @foreach($classroom as $item)
                      <tr>
                        ...
                        <form action=" {{ route('classroom.destroy', $item->id) }} " method="POST">
                              @csrf
                              @method('DELETE')
                                <a href="javascript:void(0)" onclick="editClassRoom({{$item->id}})" class="btn btn-warning btn-sm"><i class="fas fa-pencil-alt"></i></a>
        ...
        <div class="modal fade" tabindex="-1" role="dialog" id="editClassRoomModal">
          <div class="modal-dialog" role="document">
            <div class="modal-content">
              ...
              @csrf
                <div class="modal-body">
                    <input type="hidden" id="class_room_id" name="class_room_id">
